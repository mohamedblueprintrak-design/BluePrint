...
const NOTIFICATION_ICONS: Record<NotificationType, React.ReactNode> = {
  ...
  system: <Settings className="w-4 h-4 text-slate-400" />,
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenuContent 
        ...
        className="w-80 md:w-96 bg-slate-900 border-slate-800 p-0"
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-slate-800">
          <DropdownMenuLabel className="text-white p-0">
            الإشعارات
            {unreadCount > 0 && (
              <Badge variant="secondary" className="ms-2 bg-slate-700 text-slate-300">
                {unreadCount} جديد
              </Badge>
            )}
          </DropdownMenuLabel>
            ...
            <div className="py-2">
              {notifications.slice(0, 10).map((notification: Notification, index: number) => (
                <div key={notification.id}>
                  {index > 0 && <Separator className="my-1 bg-slate-800" />}
                  <DropdownMenuItem
                    className={`flex items-start gap-3 p-3 cursor-pointer hover:bg-slate-800 focus:bg-slate-800 border-l-2 ${PRIORITY_COLORS[notification.priority]} ${!notification.isRead ? 'bg-slate-800/50' : ''}`}
                    ...
                    {notification.actionUrl ? (
                      <Link href={notification.actionUrl} className="flex items-start gap-3 w-full">
                        <div className="flex-shrink-0 mt-0.5">
                          {NOTIFICATION_ICONS[notification.notificationType] || <Bell className="w-4 h-4 text-slate-400" />}
                        </div>
