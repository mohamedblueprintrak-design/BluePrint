  ...
  return (
    <div className={cn('space-y-6', className)}>
          ...
          <div className="flex items-end gap-3 pt-4" style={{ height: 200 }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="flex-1 space-y-1">
                ...
                <div
                  className="w-full rounded-t-sm bg-slate-800 animate-pulse"
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
        ...
        <div className="px-6 py-3 border-b border-slate-700/50 grid grid-cols-5 gap-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <Skeleton key={i} className={cn('h-4 w-full', SK)} />
          ))}
        </div>
        {Array.from({ length: 6 }).map((_, r) => (
          <div key={r} className="px-6 py-3 border-b border-slate-700/50 last:border-b-0 grid grid-cols-5 gap-4">
